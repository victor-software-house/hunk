...
describe("PTY navigation", () => {
  ...
  test("an applied History range stays highlighted after the review reload", async () => {
    ...
    try {
      ...
      const currentBranch = history.match(/\* ([^\s]+)/)?.[1];
      ...
      const preview = await harness.waitForSnapshot(
        session,
        (text) => text.includes(`H * ${currentBranch}`) && text.includes(`B ${initial} initial`),
        5_000,
      );
      expect(preview).toContain(`H * ${currentBranch}`);
      ...
      const applied = await harness.waitForSnapshot(
        session,
        (text) =>
          text.includes("Choose base") &&
          text.includes(`H * ${currentBranch}`) &&
          text.includes(`B ${initial} initial`),
        10_000,
      );
      expect(applied).toContain(`H * ${currentBranch}`);
