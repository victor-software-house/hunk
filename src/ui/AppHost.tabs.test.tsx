...
describe("AppHost review tabs", () => {
  test("agent commands add, select, rename, and close independently registered reviews", async () => {
    ...
    try {
      ...
      await flush(setup);
      await act(async () => setup.mockInput.typeText("l"));
      await flush(setup);
      ...
      expect(frame).toContain("filter: one");
      expect(frame).toContain("- export const one = 1;");
      expect(frame).not.toContain("1 - export const one = 1;");
      ...
      expect(frame).toContain("›  github-dark-dimmed");
      await act(async () => setup.mockInput.pressEscape());
      await flush(setup);
      ...
      await act(async () => {
        await client.dispatch({
          ...
          requestId: "reselect",
          command: "select_review_tab",
          input: { sessionId: client.registration().sessionId, tab: added.tab.tabId },
        });
      });
      await flush(setup);
      expect(setup.captureCharFrame()).toContain("export const two = 2;");
