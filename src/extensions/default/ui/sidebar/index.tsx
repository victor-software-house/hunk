      ...
      {activeTab === "files" ? (
        ...
        >
          ...
          {historyRows.map((row, index) => {
            ...
            const rangeStart = Math.min(rangeBaseIndex, historyIndex);
            const rangeEnd = Math.max(rangeBaseIndex, historyIndex);
            const inRange = rangeBaseIndex >= 0 && index >= rangeStart && index <= rangeEnd;
            const base = index === rangeBaseIndex;
            const marker = base
              ? "B"
              : rangeBase && current
                ? "H"
