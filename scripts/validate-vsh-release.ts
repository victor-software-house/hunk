...
export async function validateVshRelease(options: ValidateVshReleaseOptions = {}) {
  ...
  const originMainSha = (
    await $`git rev-parse origin/main`.cwd(repoRoot).env(gitEnv).text()
  ).trim();
  ...
  if (eventName === "workflow_dispatch") {
    ...
    if (subject !== "chore(release): version packages") {
      if (releaseSha !== originMainSha || subject !== "fork-tooling") {
        throw new Error(`Manual release SHA has unexpected subject: ${subject}`);
      }
      const distributionSubject = (
        await $`git log -1 --pretty=%s HEAD^`.cwd(repoRoot).env(gitEnv).text()
      ).trim();
      if (distributionSubject !== "vsh-distribution") {
        throw new Error(`Forkctl release has unexpected owning patch: ${distributionSubject}`);
      }
      const changed = new Set(
        (await $`git diff --name-only HEAD^^ HEAD^`.cwd(repoRoot).env(gitEnv).text())
          .trim()
          .split("\n"),
      );
      for (const required of ["package.json", "CHANGELOG.md"]) {
        if (!changed.has(required)) {
          throw new Error(`Forkctl vsh-distribution patch did not change ${required}.`);
        }
      }
      const previousManifest = JSON.parse(
        await $`git show HEAD^^:package.json`.cwd(repoRoot).env(gitEnv).text(),
      ) as { version: string };
      if (previousManifest.version === manifest.version) {
        throw new Error(`Forkctl vsh-distribution did not change ${manifest.version}.`);
      }
    }
  }
