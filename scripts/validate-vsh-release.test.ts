...
function createReleaseRepo(
  changelogBody = "- Scoped release.\n",
  releaseSubject = "chore(release): version packages",
) {
  ...
  git(repo, "add", ".");
  git(repo, "commit", "-m", releaseSubject);
  git(repo, "push", "origin", "main");
  return repo;
}

/** Add the bookkeeping tip produced when Forkctl absorbs generated release metadata. */
function createForkctlReleaseRepo() {
  const repo = createReleaseRepo("- Scoped release.\n", "vsh-distribution");
  mkdirSync(path.join(repo, "patches"));
  writeFileSync(path.join(repo, "patches", "fork.yaml"), "bookkeeping_patch: fork-tooling\n");
  git(repo, "add", ".");
  git(repo, "commit", "-m", "fork-tooling");
  git(repo, "push", "origin", "main");
  return repo;
}
...
describe("validateVshRelease", () => {
  ...
  test("accepts an exact Forkctl stack whose distribution patch owns the release", async () => {
    const repo = createForkctlReleaseRepo();
    await expect(
      validateVshRelease({ repoRoot: repo, eventName: "workflow_dispatch" }),
    ).resolves.toMatchObject({ version: "0.18.0-beta.1" });
  });
